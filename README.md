...
一个尽量简单的 JavaScript agent loop 示例。
...
- `toolHandlers`：把 tool 名字映射到本地 JS 函数。
...
- [chrome-mcp-agent.mjs](/Users/gaoyinrun/Desktop/qy/simple-agent/chrome-mcp-agent.mjs)
...
- `agent.mjs`：tool 是你自己在 JS 里手写的本地函数
- `chrome-mcp-agent.mjs`：tool 来自本机 MCP server
...
- 默认 `baseURL` 设成了 `https://api.stepfun.ai/step_plan/v1`，这是 StepFun 面向代码 Agent / 工具接入文档里给出的 OpenAI Compatible 地址
